...
public class RecyclerListView extends RecyclerView {
    ...
    private RecyclerListViewHeader mHeader;
    private RecyclerListViewFooter mFooter;
    ...
    private void init(Context context) {
        ...
        if (pullRefreshEnabled) {
            RecyclerListViewHeader recyclerViewHeader = new RecyclerListViewHeader(mContext);
            mHeaderViews.add(0, recyclerViewHeader);
            mHeader = recyclerViewHeader;
        }
        mFooter = new RecyclerListViewFooter(mContext);
        mFooter.setVisibility(GONE);
    }

    //添加头部文件的时候 判断有没有刷新头
    public void addHeaderView(View view) {
        if (pullRefreshEnabled && !(mHeaderViews.get(0) instanceof RecyclerListViewHeader)) {
            RecyclerListViewHeader recyclerViewHeader = new RecyclerListViewHeader(mContext);
            mHeaderViews.add(0, recyclerViewHeader);
            mHeader = recyclerViewHeader;
        }
    ...
    public void loadMoreComplete() {//上拉加载完成后的   隐藏上拉加载布局
        if (previousTotal < getLayoutManager().getItemCount()) {
            mFooter.setState(RecyclerListViewFooter.STATE_COMPLETE);
        }
...
//        mWrapAdapter.notifyDataSetChanged();
    ...
    public void onScrollStateChanged(int state) {
        ...
        if (state == RecyclerView.SCROLL_STATE_IDLE && mLoadingListener != null && loadingMoreEnabled) {
            ...
            if (layoutManager.getChildCount() > 0
                    ...
                    && mHeader.getState() < RecyclerListViewHeader.STATE_REFRESHING) {

                mFooter.setState(RecyclerListViewFooter.STATE_LOADING);
        ...
        switch (e.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (isOnTop() && pullRefreshEnabled) {
                    ...
                    if (mHeader.getVisiableHeight() > 0 && mHeader.getState() < RecyclerListViewHeader.STATE_REFRESHING) {
    ...
    private class WrapAdapter extends RecyclerView.Adapter<ViewHolder> {
        ...
        private RecyclerListViewFooter mFootView;

        private int headerPosition = 1;

        public WrapAdapter(ArrayList<View> headerViews, RecyclerListViewFooter footView, RecyclerView.Adapter adapter) {
